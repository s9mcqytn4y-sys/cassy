    ...
    open suspend fun getStoreProfile(storeId: String): StoreProfile? = withContext(ioDispatcher) {
        queries?.selectStoreProfileByStoreId(storeId)?.executeAsOneOrNull()?.let { record ->
            StoreProfile(
                ...
                streetAddress = record.businessAddressStreet,
                neighborhood = record.businessAddressNeighborhood,
                village = record.businessAddressVillage,
                district = record.businessAddressDistrict,
                city = record.businessAddressCity,
                province = record.businessAddressProvince,
                postalCode = record.businessPostalCode,
                phoneCountryCode = record.phoneCountryCode,
                phoneNumber = record.phoneNumber,
                businessEmail = record.businessEmail,
                legalId = record.businessLegalId,
                receiptNote = record.receiptNote,
                logoPath = record.logoAssetPath,
                showLogoOnReceipt = record.showLogoOnReceipt,
                showAddressOnReceipt = record.showAddressOnReceipt,
                showPhoneOnReceipt = record.showPhoneOnReceipt,
    ...
    open suspend fun upsertStoreProfile(profile: StoreProfile) {
        withContext(ioDispatcher) {
            queries?.upsertStoreProfile(
                ...
                profile.receiptNote,
                profile.logoPath,
                profile.showLogoOnReceipt,
                profile.showAddressOnReceipt,
                profile.showPhoneOnReceipt,
